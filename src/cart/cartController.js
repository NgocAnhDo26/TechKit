...
export const renderCartPage = (req, res) => {
  ...
  if (req.user) {
    service
      .fetchCartProducts(req.user.id)
      .then((result) => {
        const { totalPrice, totalQuantity, products } = result;
        res
          .status(200)
          .render('cart', { totalPrice, totalQuantity, products, formatPrice });
      })
      .catch((e) => {
        console.error(e);
    ...
    service
      .fetchGuestCartProducts(req.session.guestCart)
      .then((result) => {
        const { totalPrice, totalQuantity, products } = result;
        res
          .status(200)
...
router.delete('/:productId', async (req, res) => {
  ...
  if (req.user) {
    service
      .removeProductFromCart(req.user.id, productId)
      .then((result) => {
        res.status(200).json(result);
      })
      .catch((e) => {
